...
class EmpleadoController extends Controller
{
	public function index(Request $request)  // Index | Método que lista todos los Registros del Modelo:Empleado
	{
		...
		if (($atributo) && ($valorbuscado)) {
			...
			$empleados = Empleado::withTrashed()->get();
		}
	...
	public function update(Request $request)  // Update | Método que Actualiza un Registro Específico del Modelo:Empleado
	{
		return response()->json(Empleado::findOrFail($request->id)->update($request->all()));
	}
	...
	public function restoreLog($id)  // RestoreLog | Método que Restaura un Registro Específico, eliminado de forma Lógica del Modelo:Empleado
	{
		return response()->json(Empleado::onlyTrashed()->findOrFail($id)->restore());
	}
}
