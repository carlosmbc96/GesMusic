...
class UsuarioController extends Controller
{
    public function index(Request $request)  // Index | Método que lista todos los Registros del Modelo:Usuario
    {
        ...
        if ( ($atributo) && ($valorbuscado) ) {
            ...
            $usuarios=Usuario::All()->paginate(5);
        }
    ...
    public function update(Request $request,Usuario $usuario, $id)  // Update | Método que Actualiza un Registro Específico del Modelo:Usuario
    {
        return response()->json(Usuario::findOrFail($id)->update($request->all()));
    }
    ...
    public function restoreLog(Usuario $usuario, $id)  // RestoreLog | Método que Restaura un Registro Específico, eliminado de forma Lógica del Modelo:Usuario
    {
        return response()->json(Usuario::findOrFail($id)->restore());
    }
}
