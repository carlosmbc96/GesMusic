    ...
    >
      <div style="min-height: 600px">
        <div id="entrevistado_index">
          <h1 style="color: white !important">Elementos</h1>
...
export default {
  ...
  data() {
    return {
      ...
      tooltip: {
        enable: true,
        header: "Entrevistados por Estado",
        format: "${point.x} : ${point.y} Entrevistados",
          ...
          template: Vue.component("columnTemplate", {
            ...
            methods: {
              ...
              finally_method(action, error) {
                this.loading = false;
                if (!error) {
                  this.$parent.$parent.$parent.load_entrevistados();
      ...
      elementos_list: [], //* Lista de entrevistadoes que es cargada en la tabla
  ...
  methods: {
    ...
    load_entrevistados() {
      ...
      axios.post("/entrevistados/listar", { relations: ["audiovisuales"] }).then((response) => {
        ...
        this.series_data = [];
        this.elementos_list.forEach((entrevistado) => {
          let index = this.series_data.findIndex((serie) =>
            entrevistado.deleted_at != null
              ? serie.status === "Inactivo"
              : serie.status === "Activo"
          );
          if (index != -1) {
            this.series_data[index].entrevistados += 1;
          } else {
            this.series_data.push({
              status: entrevistado.deleted_at != null ? "Inactivo" : "Activo",
              entrevistados: 1,
            });
          }
        });
    ...
    panel_export_click(args) {
      let pdfExportProperties = {
        ...
        fileName: "Reporte_Entrevistados.pdf",
        ...
        header: {
          ...
          contents: [
            ...
            {
              type: "Text",
              value: "Reporte de Entrevistados",
              position: { x: 0, y: 40 },
              style: {
      ...
      if (args === "pdf") {
        ...
        excelExportProperties.fileName = "Reporte_Entrevistados.xlsx";
        this.$refs.gridObj.getColumns()[4].visible = false;
        this.$refs.gridObj.excelExport(excelExportProperties);
      } else if (args === "csv") {
        excelExportProperties.fileName = "Reporte_Entrevistados.csv";
        this.$refs.gridObj.getColumns()[4].visible = false;
