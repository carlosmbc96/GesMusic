...
export default {
      ...
      .then((response) => {
        for (let i = 0; i < response.data.length; i++) {
          this.mockData.push({
            ...
            codig: response.data[i].codigFong,
            title: response.data[i].tituloFong,
          });
  ...
  methods: {
    ...
    submit() {
      this.change_spin();
      axios
        .post("productos/actualizarRelacionesFong", {
          id: this.entity_id,
          fonogramas: this.new_relations,
        })
        .then((response) => {
