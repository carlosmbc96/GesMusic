        ...
        <a-tab-pane key="2" :disabled="tab_2">
          ...
          <a-spin :spinning="spinning">
            ...
            >
                  ...
                  <a-row style="margin-bottom: 30px">
                    <a-col span="11">
                      ...
                      <a-form-model-item style="margin-top: 20px" v-else>
                        <i
                          class="fa fa-check-square-o hidden-xs"
                          v-if="product.activoCatalbisPro === 1"
                        />
                        <i class="fa fa-square-o" v-else />
                        ¿Activo en el Catálogo de Bismusic?
                      </a-form-model-item>
                      ...
                      <a-form-model-item v-else>
                        <i
                          class="fa fa-check-square-o hidden-xs"
                          v-if="product.primeraPantProd === 1"
                        />
                        <i class="fa fa-square-o" v-else />
                        ¿Estará en la Primera Pantalla?
                      </a-form-model-item>
                    ...
                    <a-col span="11" style="float: right">
                      ...
                      <a-form-model-item style="margin-top: 20px" v-else>
                        <i
                          class="fa fa-check-square-o hidden-xs"
                          v-if="product.catalDigitalPro === 1"
                        />
                        <i class="fa fa-square-o" v-else />
                        ¿Estará en el Catálogo Digital?
                      </a-form-model-item>
              ...
              <div
                v-if="action_modal === 'crear' || action_modal === 'detalles'"
              >
                <a-row>
                  <div class="section-title" style="width: 46%">
                    <h4>Datos Descripciones</h4>
  ...
  methods: {
    ...
    validate() {
      ...
      if (!this.used) {
        ...
        if (this.active_tab === "1") {
          this.$refs.formularioproject.validate((valid) => {
            if (!valid) {
              ...
              this.$toast.warning(
                "Hay problemas en la pestaña Proyecto,<br> por favor antes de continuar revísela!",
                "¡Atención!",
                {
                  timeout: 4000,
                  id: "question",
        ...
        if (valid_form === 2) {
          this.$toast.warning(
            "Hay problemas en las pestañas Proyecto y Generales,<br> por favor antes de continuar revíselas!",
            "¡Atención!",
            {
              timeout: 4000,
              id: "question",
    ...
    confirm() {
      ...
      if (this.action_modal === "editar" || this.action_modal === "detalles") {
        ...
        if (this.product_modal.codigProd === undefined) {
          form_data.append("codigProd", "PROD-" + this.codigo);
        } else {
          this.product_modal.codigProd = "PROD-" + this.product_modal.codigProd;
          form_data.append("codigProd", this.product_modal.codigProd);
        }
    ...
    remove_interprete(item) {
      ...
      if (item.value !== "") {
        this.copia_interpretes.push(
          this.list_interpretes[
            this.find_element_interprete(item.value, this.list_interpretes)
          ]
        );
      }
      if (index !== -1) {
    ...
    change_interprete(item) {
      ...
      if (this.copia_interpretes.length === 1) {
        ...
        this.copia_interpretes.splice(
          this.find_element_interprete(item.value, this.copia_interpretes),
          1
        );
      }
      ...
      document.getElementById(item.key).blur();
    },
